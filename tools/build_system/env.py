...
class EnvironmentFiles:
    ...
    disk: Path
    # Path to a "ROM" of the original executable which is created by this script.
    # The "ROM" only contains functions and data, without ELF metadata, and is
    # what we actually compare to.
    disk_rom: Path
    # Path to the final ELF file. This is a valid shiftable ELF which contains all
    # of the game's code and data, but it is **nonmatching** and not used for comparison.
    final_elf: Path
...
class EnvironmentToolchain:
    ...
    ld_flags: list[str]
    # Base flags for creating a ROM using `objcopy`.
    objcopy_rom_flags: list[str]
    ...
    def for_version(
        ...
        return EnvironmentToolchain(
            ...
            # We need to pad about 0x40 bytes onto what `objcopy` would normally generate.
            # `bss` starts at VRAM `0x4FBD00`, but the preceding section in the original binary,
            # `sdata`, actually ends at `0x4FBCC0`.
            # Splat can't really handle this effectively.
            objcopy_rom_flags=[
                "-O binary",
                "--gap-fill 0x00",
            ],
...
class Environment:
    ...
    def generate_objcopy_rom_cmd(self, pad_to_addr: str) -> str:
        """
        Generate the full `objcopy` invocation to create a ROM for this environment,
        not including input/output parameters.
        """
        return f"{self.toolchain.objcopy_cmd} {' '.join(self.toolchain.objcopy_rom_flags)} --pad-to {pad_to_addr}"
